...
{
    static GET_ORDER_URL = '/api/invoice';
    ...
    getInvoice() {
        axios({
            method: 'GET',
            url: process.env.REACT_APP_API_URI + Profil.GET_ORDER_URL + '/' + this.invoiceId,
            headers: {
                    ...
                    <Row>
                        <Col md="12">
                            <Card className="main-card mb-3">
                                <CardBody>
                                    <CardTitle>
                                        Facture N°{this.state.invoice.id}
                                        <div className="pull-right mb-2 mr-2 badge badge-pill badge-success">{this.state.invoice.item}</div>
                                    </CardTitle>
...
const MenuProfil = (props) => {
    return (
        <Nav pills justified>
            ...
            <NavItem>
                <NavLink className={ `${props.props.location.pathname}` === `${props.props.match.url}/adresse-facturation` ? 'active' : '' } tag={(props) => <Link {...props} />} to={`${props.props.match.url}/adresse-facturation`}>
                    <i className="nav-link-icon pe-7s-settings"> </i>
